...
RemoveItemsForm::~RemoveItemsForm()
{
  delete ui;
}
...
void RemoveItemsForm::elementSelected( TreeWidgetItem* item, int column )
{
  ...
  if( item )
  {
    ...
    QStringList attributes = DataBaseInterface::instance()->attributes( m_currentElement );
...
void RemoveItemsForm::attributeActivated( const QString& attribute )
{
  ...
  QStringList attributeValues = DataBaseInterface::instance()->attributeValues( m_currentElement, m_currentAttribute );
...
void RemoveItemsForm::deleteElement( const QString& element )
{
  ...
  QStringList children = DataBaseInterface::instance()->children( currentElement );
  ...
  if( !children.isEmpty() )
  {
    ...
    {
      if( DataBaseInterface::instance()->isUniqueChildElement( currentElement, child ) )
      {
        deleteElement( child );
      }
    }
  ...
  QStringList attributes = DataBaseInterface::instance()->attributes( currentElement );

  foreach( QString attribute, attributes )
  {
    if( !DataBaseInterface::instance()->removeAttribute( currentElement, attribute ) )
    {
  ...
  if( !DataBaseInterface::instance()->removeElement( currentElement ) )
  {
    MessageSpace::showErrorMessageBox( this, DataBaseInterface::instance()->lastError() );
  }
  else
  ...
  if( DataBaseInterface::instance()->knownRootElements().contains( currentElement ) )
  {
    if( !DataBaseInterface::instance()->removeRootElement( currentElement ) )
    {
      MessageSpace::showErrorMessageBox( this, DataBaseInterface::instance()->lastError() );
    }
  }
...
void RemoveItemsForm::removeChildElement()
{
  ...
  {
    if( !DataBaseInterface::instance()->removeChildElement( m_currentElementParent, m_currentElement ) )
    {
      MessageSpace::showErrorMessageBox( this, DataBaseInterface::instance()->lastError() );
    }
    else
    {
      if( DataBaseInterface::instance()->isUniqueChildElement( m_currentElementParent, m_currentElement ) )
      {
        bool accepted = MessageSpace::userAccepted( "RemoveUnlistedElement",
                                                      "Element not used",
...
void RemoveItemsForm::updateAttributeValues()
{
  ...
  {
    bool accepted = MessageSpace::userAccepted( "UpdateEmptyAttributeValues",
  ...
  else
  {
    /* All existing values will be replaced with whatever remained in the text edit by the time the
      user was done. */
    if( !DataBaseInterface::instance()->updateAttributeValues( m_currentElement, m_currentAttribute, attributes, true ) )
    {
      MessageSpace::showErrorMessageBox( this, DataBaseInterface::instance()->lastError() );
    }
    else
...
{
  if( !DataBaseInterface::instance()->removeAttribute( m_currentElement, m_currentAttribute ) )
  {
    MessageSpace::showErrorMessageBox( this, DataBaseInterface::instance()->lastError() );
  }
  else
...
{
  QStringList knownElements = DataBaseInterface::instance()->knownElements();
  ...
  foreach( QString element, knownElements )
  {
   ...
   if( !children.isEmpty() )
   {
     foreach( QString deletedElement, m_deletedElements )
     {
       ...
       {
         if( !DataBaseInterface::instance()->removeChildElement( element, deletedElement ) )
         {
           MessageSpace::showErrorMessageBox( this, DataBaseInterface::instance()->lastError() );
         }
       }
