...
public:
  ...
  explicit SearchForm( const QList< TreeWidgetItem* >& items, QPlainTextEdit* textEdit, QWidget* parent = 0 );
...
private:
  ...
  /*! Called from within the search function, finds the match (if any) and emits the foundItem
      signal if it does. */
  void findMatchingTreeItem(  const QList< TreeWidgetItem* > matchingItems, bool ascending  );
