...
AddSnippetsForm::AddSnippetsForm( const QString& elementName, TreeWidgetItem* parentItem, QWidget* parent )
: QDialog            ( parent ),
  ui                 ( new Ui::AddSnippetsForm ),
...
AddSnippetsForm::~AddSnippetsForm()
{
  delete ui;
}
...
void AddSnippetsForm::attributeChanged( QTableWidgetItem* item ) const
{
  if( !m_treeItemActivated )
  {
    ...
    ComboBox* attributeValueCombo = dynamic_cast< ComboBox* >( ui->tableWidget->cellWidget( item->row(), COMBOCOLUMN ) );
  ...
  for( int i = 0; i < attributes.size(); ++i )
  {
    for( int j = 0; j < ui->tableWidget->rowCount(); ++j )
    {
      ...
      if( attributeName == attributes.item( i ).nodeName() )
      {
        ...
        ComboBox* comboBox = dynamic_cast< ComboBox* >( ui->tableWidget->cellWidget( j, COMBOCOLUMN ) );
...
{
  QList< TreeWidgetItem* > includedItems = ui->treeWidget->includedTreeWidgetItems();
  ...
  for( int i = 0; i < ui->spinBox->value(); ++i )
  {
    ...
    for( int j = 0; j < includedItems.size(); ++j )
    {
      ...
      for( int k = 0; k < attributes.size(); ++k )
      {
        ...
        DataBaseInterface::instance()->updateAttributeValues( elementName, attr.name(), QStringList( attributeValue ) );
      }
    }
