...
/*! Responsible for the display of error messages and messages requiring user
 * input with the option to remember the user's preference.  Some message
 * prompts displayed via this namespace contain the option to remember the
 * user's preference.  In cases where a user preference can be saved,
 * MessageSpace will persist the changes to whatever medium exists on the
 * platform it's running on (Windows registry, Mac XML, Unix ini).  This space
 * is furthermore responsible for the display of all error messages, but not ALL
 * messages (some messages always need to be shown and make more sense
...
/*! This function will return the saved user preference (if there is one),
    ...
    @param uniqueMessageKey - a unique name representing a specific message,
   this name is saved to the registry/xml/ini file
   ...
   message
    ...
    @param saveCancel - if this value is set to "false", "Cancel"-ed user
   preferences will not be saved, irrespective of whether or not the
   user ticked the relevant box. */
bool userAccepted(const QString &uniqueMessageKey, const QString &heading,
                  const QString &text, ButtonCombo buttons,
                  Buttons defaultButton, Icon icon = NoIcon,
