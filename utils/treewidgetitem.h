...
/// Used by DomTreeWidget, each TreeWidgetItem can be associated with a QDomElement.
...
/**
    ...
    Some specialisation specific to AddSnippetsForm was added to this class (all functions related to
    ...
    tried to use maps confined to AddSnippetForm objects, but to no avail).
*/
...
public:
  ...
  /*! Sets the "exclude" flag used to determine whether or not the element must be included in DomTreeWidget's
  ...
  /*! Adds "attribute" to a list of attributes whose values must be incremented when multiple snippets
      ...
      to AddSnippetForm objects, but to no avail).
