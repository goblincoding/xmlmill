...
BatchProcessHelper::BatchProcessHelper(const QDomDocument *domDoc,
    ...
    : m_stringListSeparator(stringListSeparator),
      ...
      m_newElementAttributesToAdd(), m_elementsToUpdate(),
      m_elementChildrenToUpdate(), m_elementAttributesToUpdate(),
      m_newAttributeKeysToAdd(), m_newAssociatedElementsToAdd(),
      m_newAttributeValuesToAdd(), m_attributeKeysToUpdate(),
      m_associatedElementsToUpdate(), m_attributeValuesToUpdate(), m_unsorted(),
      m_records() {
...
void BatchProcessHelper::createRecord(const QDomElement &element) {
  ...
  for (int i = 0; i < attributeNodes.size(); ++i) {
    ...
    if (!attribute.isNull()) {
      record.attributes.insert(attribute.name(),
                               QStringList(attribute.value()));
    }
  }

  /* Collect all the first level (element) children associated with this
   * element. */
...
void BatchProcessHelper::sortRecords() {
   ...
   * and consolidate the entries per element. */
  foreach(QString element, m_unsorted.uniqueKeys()) {
    ...
    /* If we have more than one record for the same element, then we need to
     * consolidate all the attributes associated with this particular element.
     */
    if (duplicateRecords.size() > 1) {
      ...
      for (int i = 0; i < duplicateRecords.size(); ++i) {
        ...
        /* Don't consolidate the attribute values here (see above comment) but
         * since we're already iterating through the list, we may as well
         * consolidate the children while we're at it (they are relatively
         * simple to handle compared to the attribute value maps). */
        record.children.append(duplicateRecords.at(i).children);
      }

      record.children.removeDuplicates();

      /* Now we can sort out the chaos. For each unique attribute, we will
       * obtain the associated QStringList(s) of attribute values and
       * consolidate the lot. */
...
void BatchProcessHelper::createVariantLists() {
  ...
  foreach(QVariant var, m_newElementsToAdd) {
    ...
    if (!m_records.value(element).children.isEmpty()) {
      m_newElementChildrenToAdd
          << m_records.value(element).children.join(m_stringListSeparator);
    } else {
      /* To keep the indices in sync, the following will result in a NULL value
       * being bound to the relevant prepared query on the DB side (we need to
       * have exactly the same number of items in all lists associated with the
       * "new elements" in order for the batch bind to succeed). The same
       * argument holds for the "elements to update" and two types of attribute
       * lists below. */
  ...
  /* Separate the new attribute keys and associated values from the existing
   * ones. */
  foreach(QString element, elementNames) {
    ...
    foreach(QString attribute, attributeNames) {
      QStringList attributeValues =
          m_records.value(element).attributes.value(attribute);
