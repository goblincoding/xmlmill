...
private slots:
  /*! Triggered as soon as the main event loop is entered (via a connection to a single shot
      timer in the constructor). This function ensures that DataBaseInterface is successfully
      initialised and prompts the user to select a database for the current session. */
  void initialise();

  /*! Connected to the UI tree widget's "CurrentItemChanged( TreeWidgetItem*, int )" signal.
      \sa elementSelected */
  void elementChanged( TreeWidgetItem* item, int column );

  /*! Connected to the UI tree widget's "CurrentItemSelected( TreeWidgetItem*, int )" signal.
  ...
  /*! Connected to ComboBox's "currentIndexChanged( QString )" signal.
  ...
  /*! Connected to the signal mapper's "mapped( QWidget* )" signal which is emitted every
      time a ComboBox is activated (whenever the user enters or otherwise activates a combo box).
  ...
  /*! Connected to DBSessionManager's "activeDatabaseChanged( QString )" signal.
      \sa addNewDatabase
      \sa addExistingDatabase
      \sa removeDatabase
      \sa switchActiveDatabase */
  ...
  /*! Connected to the "Add Snippet" button's "clicked()" signal. This function creates and displays
      an instance of AddSnippetsForm to allow the user to add one (or more) XML snippets to the active
      document.
      \sa addElementToDocument
      \sa insertSnippet */
  void addSnippetToDocument();

  /*! Connected to the AddSnippetsForm's "snippetAdded()" signal.  This function updates the GUI whenever
  ...
  /*! Triggered by the "Remove Items" UI action. This function creates and displays an instance of
      RemoveItemsForm to allow the user to remove elements and/or attributes from the active database.
      \sa addItemsToDB */
  void removeItemsFromDB();

  /*! Triggered by the "Add Items" UI action. This function creates and displays an instance of
      AddItemsForm to allow the user to add elements and/or attributes to the active database.
      \sa removeItemsFromDB */
  void addItemsToDB();

  /*! Connected to the "Find in Document" UI action. This function creates and displays an instance of
      SearchForm to allow the user to search for specific strings in the current document.
      \sa itemFound */
  void searchDocument();

  /*! Connected to SearchForm's "foundItem" signal.  This slot sets the found item as active.
      \sa searchDocument */
  void itemFound( TreeWidgetItem* item );

  /*! Connected to PlainTextEdit's "commentOut" signal. Removes the items with indices matching those
  ...
  /*! Connected to PlainTextEdit's "manualEditAccepted()" signal. Rebuilds the XML hierarchy for
...
private:
  /*! Creates a new DBSessionManager and connects its signals to the relevant slots.
      \warning The calling function is responsible for clean-up! */
  DBSessionManager* createDBSessionManager();
