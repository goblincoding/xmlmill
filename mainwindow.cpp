  ...
  m_comboBoxes              ()
{
  ...
  ui->tableWidget->horizontalHeader()->setFont( QFont( GlobalSpace::FONT, GlobalSpace::FONTSIZE ) );
  ...
  connect( ui->treeWidget, SIGNAL( CurrentItemSelected( TreeWidgetItem*, int ) ), this, SLOT( elementSelected( TreeWidgetItem*, int ) ) );
  connect( ui->treeWidget, SIGNAL( CurrentItemChanged( TreeWidgetItem*, int ) ), this, SLOT( elementChanged( TreeWidgetItem*, int ) ) );
...
void MainWindow::initialise()
{
  ...
  if( !DataBaseInterface::instance()->isInitialised() )
  {
  ...
  DBSessionManager* manager = createDBSessionManager();
...
void MainWindow::elementSelected( TreeWidgetItem* item, int column )
{
  ...
  if( item )
  {
    ...
    for( int i = 0; i < attributeNames.count(); ++i )
    {
      ...
      attributeCombo->addItems( DataBaseInterface::instance()->attributeValues( elementName, attributeNames.at( i ) ) );
    ...
    ui->addElementComboBox->addItems( DataBaseInterface::instance()->children( elementName ) );
...
void MainWindow::attributeChanged( QTableWidgetItem* tableItem )
{
  ...
  if( !m_wasTreeItemActivated && !m_newAttributeAdded )
  {
    ...
    if( tableItem->text() != m_activeAttributeName )
    {
      ...
      if( DataBaseInterface::instance()->updateElementAttributes( treeItem->name(), QStringList( tableItem->text() ) ) )
      {
        ...
        if( m_activeAttributeName != EMPTY )
        {
          ...
          QStringList attributeValues = DataBaseInterface::instance()->attributeValues( treeItem->name(),
                                                                                          m_activeAttributeName );

          if( !DataBaseInterface::instance()->updateAttributeValues( treeItem->name(), tableItem->text(), attributeValues ) )
          {
            MessageSpace::showErrorMessageBox( this, DataBaseInterface::instance()->lastError() );
          }
        }
    ...
    ComboBox* attributeValueCombo = dynamic_cast< ComboBox* >( ui->tableWidget->cellWidget( tableItem->row(), VALUESCOLUMN ) );
...
void MainWindow::attributeValueChanged( const QString& value )
{
  ...
  if( !m_wasTreeItemActivated && !value.isEmpty() )
  {
    ...
    QStringList attributeValues = DataBaseInterface::instance()->attributeValues( treeItem->name(), currentAttributeName );

    if( !attributeValues.contains( value ) )
    {
      if( !DataBaseInterface::instance()->updateAttributeValues( treeItem->name(),
...
void MainWindow::setCurrentComboBox( QWidget* combo )
{
...
bool MainWindow::openXMLFile()
{
  ...
  QString fileName = QFileDialog::getOpenFileName( this, "Open File", GlobalSpace::lastUserSelectedDirectory(), "XML Files (*.*)" );
  ...
  if( !m_busyImporting )
  {
    ...
    {
      bool accepted = MessageSpace::userAccepted( "QueryImportXML",
                                                    "Import document?",
...
{
  QString file = QFileDialog::getSaveFileName( this, "Save As", GlobalSpace::lastUserSelectedDirectory(), "XML Files (*.*)" );
...
void MainWindow::addNewDatabase()
{
  DBSessionManager* manager = createDBSessionManager();
...
void MainWindow::addExistingDatabase()
{
  DBSessionManager* manager = createDBSessionManager();
...
void MainWindow::switchActiveDatabase()
{
  DBSessionManager* manager = createDBSessionManager();
...
void MainWindow::activeDatabaseChanged( QString dbName )
{
  if( ui->treeWidget->empty() )
  {
    resetDOM();
...
void MainWindow::addElementToDocument()
{
  ...
  if( !elementName.isEmpty() )
  {
    ...
    QStringList attributes = DataBaseInterface::instance()->attributes( elementName );
...
void MainWindow::addSnippetToDocument()
{
  ...
  if( elementName.contains( QRegExp( LEFTRIGHTBRACKETS ) ) )
  {
    ...
    AddSnippetsForm* dialog = new AddSnippetsForm( elementName.remove( QRegExp( LEFTRIGHTBRACKETS ) ),
                                                       ui->treeWidget->CurrentItem()->Parent(),
                                                       this );
    connect( dialog, SIGNAL( snippetAdded( TreeWidgetItem*, QDomElement ) ), this, SLOT( insertSnippet( TreeWidgetItem*, QDomElement ) ) );
    dialog->exec();
  }
  else
  {
    /* Qt::WA_DeleteOnClose flag set. */
    AddSnippetsForm* dialog = new AddSnippetsForm( elementName,
...
{
  if( !DataBaseInterface::instance()->hasActiveSession() )
  {
    MessageSpace::showErrorMessageBox( this, "No active profile." );
    return;
  }
  ...
  if( !ui->treeWidget->empty() )
  {
    MessageSpace::showErrorMessageBox( this, "For practical reasons, items cannot be removed when documents are open.\n"
...
void MainWindow::addItemsToDB()
{
  if( !DataBaseInterface::instance()->hasActiveSession() )
  {
    MessageSpace::showErrorMessageBox( this, "No active profile, please set one (Edit->Switch Profile)." );
    return;
  }
  ...
  if( profileWasEmpty )
  {
    ui->addElementComboBox->addItems( DataBaseInterface::instance()->knownRootElements() );
    toggleAddElementWidgets();
  }
}
...
{
  /* Delete on close flag set (no clean-up needed). */
  SearchForm* form = new SearchForm( ui->treeWidget->allTreeWidgetItems(), ui->dockWidgetTextEdit, this );
  connect( form, SIGNAL( foundItem( TreeWidgetItem* ) ), this, SLOT( itemFound( TreeWidgetItem* ) ) );
  form->exec();
}
...
void MainWindow::showMainHelp()
{
  ...
  if( !file.open( QIODevice::ReadOnly | QIODevice::Text ) )
  {
    MessageSpace::showErrorMessageBox( this, QString( "Failed to open \"Help\" file: [%1]" ).arg( file.errorString() ) );
  }
  else
  {
...
DBSessionManager* MainWindow::createDBSessionManager()
{
  /* Clean-up is the responsibility of the calling function. */
  DBSessionManager* manager = new DBSessionManager( this );
...
void MainWindow::querySetActiveSession( QString reason )
{
  while( !DataBaseInterface::instance()->hasActiveSession() )
  {
    MessageSpace::showErrorMessageBox( this, reason );
    DBSessionManager* manager = createDBSessionManager();
    manager->selectActiveDatabase();
    delete manager;
  }
...
void MainWindow::readSettings()
{
  ...
  ui->actionRememberWindowGeometry->setChecked( GlobalSpace::useWindowSettings() );
...
void MainWindow::saveSettings()
{
  ...
  GlobalSpace::setUseWindowSettings( ui->actionRememberWindowGeometry->isChecked() );
}
...
void MainWindow::queryRestoreFiles()
{
  ...
  QStringList tempFiles = QDir::current()
    ...
    .filter( QString( "%1_temp" )
             .arg( DataBaseInterface::instance()->activeSessionName().remove( ".db" ) ) );
